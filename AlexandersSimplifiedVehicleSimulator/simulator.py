...
class Simulator():
    def __init__(self, vehicle: Otter, map: SimpleMap, eta_i=np.zeros((6, 1), float), fps=30) -> None:
        ...
        # Initial conditions
        self.eta_i = eta_i  # Save initial pose
        self.eta = eta_i    # Initialize pose
    ...
    def simulate(self):
        ...
        self.screen = pygame.display.set_mode([map.BOX_WIDTH, map.BOX_LENGTH])
        self.screen.fill(map.OCEAN_BLUE)
    ...
    def render(self):
        ...
        if self.vehicle != None:
            vessel_image, vessel_shape = self.vehicle.render_update(
...
def test_simulator():
    ...
    map = SimpleMap()
    simulator = Simulator(None, map, fps)
    simulator.simulate()
